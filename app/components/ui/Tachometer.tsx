...
export function Tachometer({ score, maxScore, size = 260 }: TachometerProps) {
    ...
    <div style={{ width: size, height: size }} className="relative select-none">
      <svg viewBox="0 0 200 200" width={size} height={size}>
        <defs>
          ...
          <linearGradient id="arcGradient" gradientUnits="userSpaceOnUse" x1="20" y1="100" x2="180" y2="100">
        ...
        {/* Background disc */}
        <circle cx={CX} cy={CY} r={OUTER_R} fill="url(#gaugeGlow)" />
        ...
        {ticks.map((i) => {
          ...
          const outer = polarToCartesian(CX, CY, OUTER_R - 2, angle);
          const label = polarToCartesian(CX, CY, OUTER_R - 18, angle);
        ...
        <motion.g style={{ rotate: springAngle, originX: "100px", originY: "100px" }}>
          <line x1={CX} y1={CY - 8} x2={CX} y2={CY - 65} stroke="#ef4444" strokeWidth="2.5" strokeLinecap="round" filter="url(#needleGlow)" />
        ...
        <text x={CX} y={CY + 46} textAnchor="middle" fontSize="4" fill="#444" fontFamily="monospace" letterSpacing="1">
          ×1000 RPT
        </text>
        <text x={CX} y={CY - 36} textAnchor="middle" fontSize="4" fill="#333" fontFamily="'Outfit', sans-serif" letterSpacing="3" fontWeight="700">
          AUTOGUESSR
        </text>
      </svg>
    </div>
