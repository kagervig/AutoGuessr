  ...
  return (
    <div className="text-xs">
      <div className="flex justify-between mb-0.5">
        ...
        <span
          className={confirmed ? "text-green-600 font-medium" : "text-gray-400"}
        >
          {count}/5
        </span>
      </div>
...
export default function StagingImagePanel() {
  ...
  const [statusFilter, setStatusFilter] = useState<StagingStatus | "ALL">(
    "ALL",
  );
  ...
  async function autoUpdate() {
    ...
    const res = await fetch("/api/admin/staging/auto-update", {
      method: "POST",
    });
    ...
    if (res.ok) {
      ...
      const parts: string[] = [];
      if (updated > 0)
        parts.push(`Updated ${updated} image${updated !== 1 ? "s" : ""}`);
      if (vehicleUpdated > 0)
        parts.push(
          `filled fields for ${vehicleUpdated} from existing vehicles`,
        );
      if (skipped > 0) parts.push(`${skipped} skipped (unknown make)`);
      setAutoUpdateResult(
        parts.length > 0 ? parts.join(", ") + "." : "Nothing to update.",
      );
  ...
  async function deleteUnusedRejected() {
    if (
      !confirm(
        "Delete all unused rejected images from Cloudinary and the database? This cannot be undone.",
      )
    )
      return;
    setDeletingUnused(true);
    setDeleteResult(null);
    const res = await fetch("/api/admin/staging/bulk-unused-rejected", {
      method: "DELETE",
    });
    ...
    if (res.ok) {
      ...
      setDeleteResult(
        `Deleted ${data.deleted}${data.skipped ? `, skipped ${data.skipped} (referenced in game history)` : ""}`,
      );
  ...
  function patchImage(updated: Partial<StagingImage> & { id: string }) {
    setImages((imgs) =>
      imgs.map((img) => (img.id === updated.id ? { ...img, ...updated } : img)),
    );
  }

  function removeImage(
    id: string,
    oldStatus: StagingStatus,
    newStatus?: StagingStatus,
  ) {
    setImages((imgs) =>
      statusFilter === "ALL"
        ? imgs.map((img) =>
            img.id === id && newStatus ? { ...img, status: newStatus } : img,
          )
  ...
  async function saveMultiple() {
    ...
    try {
      for (const id of selectedIds) {
        ...
        if (!img.admin.bodyStyle)
          payload.bodyStyle = editForm.bodyStyle || null;
        if (!img.admin.rarity) payload.rarity = editForm.rarity || null;
        if (!img.admin.era) payload.era = editForm.era || null;
        if (!img.admin.regionSlug)
          payload.regionSlug = editForm.regionSlug || null;
        if (!img.admin.countryOfOrigin)
          payload.countryOfOrigin = editForm.countryOfOrigin || null;
        if (!img.admin.categories?.length)
          payload.categories = editForm.categories;
        if (!img.admin.notes) payload.notes = editForm.notes || null;
        if (!img.admin.copyrightHolder)
          payload.copyrightHolder = editForm.copyrightHolder || null;
  ...
  function isMissingField(img: StagingImage, field: string): boolean {
    switch (field) {
      case "categories":
        return img.admin.categories.length === 0;
      case "make":
        return !img.admin.make && !img.confirmed.make && !img.ai.make;
      case "model":
        return !img.admin.model && !img.confirmed.model && !img.ai.model;
      case "year":
        return !img.admin.year && !img.confirmed.year && !img.ai.year;
      case "era":
        return !img.admin.era;
      case "region":
        return !img.admin.regionSlug;
      case "country":
        return !img.admin.countryOfOrigin;
      default:
        return false;
    }
  }
  ...
  function isIncomplete(img: StagingImage) {
    ...
    const missingRequired =
      ...
      !model ||
      !year ||
      !img.admin.regionSlug ||
      !img.admin.countryOfOrigin;
  ...
  const filterMakeOptions = useMemo(
    () => autocomplete.makeOptions.sort(),
    [autocomplete.makeOptions],
  );
  const filterModelOptions = useMemo(
    () => autocomplete.modelOptions.sort(),
    [autocomplete.modelOptions],
  );
  ...
  const displayedImages = images.filter((img) => {
    ...
    if (missingFieldFilter && !isMissingField(img, missingFieldFilter))
      return false;
    return true;
  });
                      ...
                      >
                        ...
                        >
                          <input
                            ...
                            style={
                              selectedIds.includes(img.id) ? { opacity: 1 } : {}
                            }
                          />
                        </div>
                        {loadedImageIds.has(img.id) ? (
                          <div
                            ...
                            onClick={(e) => {
                              ...
                              handleImageClick(img, e.shiftKey);
                            }}
                          >
                            <Image
                              fill
                        ...
                        <div
                          className="p-1.5 bg-white select-none"
                          onClick={(e) => {
                            if (e.shiftKey) e.preventDefault();
                            handleImageClick(img, e.shiftKey);
                          }}
                        >
                          ...
                          {img.suggestionCount > 0 && (
                            <p className="text-[10px] text-blue-500">
                              {img.suggestionCount} suggestion
                              {img.suggestionCount !== 1 ? "s" : ""}
                            </p>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="p-4 space-y-4">
                              ...
                              <div>
                                ...
                                {selected.agreements.model.value && (
                                  <AgreementBar
                                    ...
                                    confirmed={
                                      selected.agreements.model.confirmed
                                    }
                                  />
                                )}
                                {selected.agreements.year.value && (
                                  <AgreementBar
                                    ...
                                    confirmed={
                                      selected.agreements.year.confirmed
                                    }
                                  />
                                )}
                                {selected.agreements.trim.value && (
                                  <AgreementBar
                                    ...
                                    confirmed={
                                      selected.agreements.trim.confirmed
                                    }
                                  />
                                )}
                              </div>
                    ...
                    {isPublished && (
                      <p className="text-sm text-amber-700 bg-amber-50 rounded px-3 py-2">
                        This image is published. Edit it directly from the
                        Images tab.
                      </p>
                    )}
                    ...
                    <StagingEditFields
                      ...
                      copyrightHolderOptions={
                        autocomplete.copyrightHolderOptions
                      }
                    ...
                    <button
                      onClick={() =>
                        isMultiSelect ? saveMultiple() : saveEdit(selected!.id)
                      }
                    ...
                    {selected && (
                      <div className="space-y-2 pt-2 border-t border-gray-100">
                        ...
                        {selected.status !== "COMMUNITY_REVIEW" && (
                          <button
                            onClick={() =>
                              setStatus(selected.id, "COMMUNITY_REVIEW")
                            }
                            disabled={saving || publishing}
                        ...
                        {selected.status !== "PENDING_REVIEW" &&
                          ...
                          selected.status !== "REJECTED" && (
                            <button
                              onClick={() =>
                                setStatus(selected.id, "PENDING_REVIEW")
                              }
                              disabled={saving || publishing}
